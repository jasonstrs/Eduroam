<?php
    ...
    if(valider("connecte","SESSION") && (valider("admin","SESSION")==1 || getDroitByUser(valider("idUser","SESSION"), "annonce"))) {
		$annonce = getAnnonceById(valider("id","GET"));
?>
...
<input type="hidden" id="id" value="<?php echo valider("id","GET")?>">
...
<button id="submit" type="button" class="btn btn-outline-secondary mt-3 offset-10">Enregistrer l'annonce</button>
...
<script>
	...
	$(function(){
		$(".ql-editor").html(`<?php echo htmlspecialchars_decode($annonce[0]["contenu"]); ?>`);
	});
